...
function is(...x) {
  const members = {
    ...
    inRange: (lo, hi) => x.every((v) => lo <= v <= hi),
    ...
    RealNum: () => x.every((v) => typeof v === "number" && !isNaN(v)),
    Integer: () => x.every((v) => typeof v === "number" && Number.isInteger(v)),
    Floating: () => x.every((v) => typeof v === "number" && !Number.isInteger(v)),
    ...
    NaN: () => x.every((v) => typeof v === "number" && isNaN(v)),
